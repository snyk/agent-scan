...
- `0.6.5` Ask for confirmation before contacting each configured remote MCP server in interactive scans; unattended scans keep their existing consent behavior. Stop following HTTP redirects for remote MCP connections and refuse link-local and known cloud-metadata addresses, while still allowing loopback and private network addresses.
