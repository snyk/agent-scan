...
- **Skills:** included by default; use `--no-skills` for MCP-only scanning. Directory skills collect only files within the resolved skill root; symlinked roots and links within that root are supported. Collection is limited to 10 path components and 20 MiB per file. Skipped entries are listed in the skill's `X002` / `skill_scan_error` message without discarding readable files. Escaping and broken links also appear in `files[]` as explicit synthetic markers with a coarse target category only; target contents and resolved paths are never included, and linked external directories are not traversed.
