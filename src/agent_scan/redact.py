...
def redact_error_text(text: str | None) -> str | None:
    ...
    then ``Bearer <token>`` values -- which OAuth access tokens can surface
    in a 401 dump but which the keyword/entropy plugins below don't reliably
    catch outside a key-value context -- so the detect-secrets pass runs last
    over already-shortened, already-scrubbed text.
    """
    return redact_text(redact_bearer_tokens(redact_absolute_paths(text)))
