...
Claude Desktop plugin discovery is macOS-only: it reads `~/Library/Application Support/Claude/local-agent-mode-sessions/*/*/rpm/manifest.json` and scans only the listed installed plugin folders for skills and MCP servers. For Claude Code, Guard's install and session-start discovery events also report logged connectors from `claude-code-sessions/*/*/local_*.json` and `local-agent-mode-sessions/*/*/local_*.json`, grouped by organization and deduplicated by connector UUID using the newest session metadata. Only each connector's UUID, name, URL, tool definitions and instructions are sent; other fields such as headers are dropped, and URL query values are redacted. On Windows, connector discovery uses `~/AppData/Roaming/Claude` with the same assumed session layout, which has not yet been verified on a Windows machine. Windows plugin discovery remains unsupported.
